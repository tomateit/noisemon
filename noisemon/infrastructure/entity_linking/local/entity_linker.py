...
class EntityLinkerImpl(EntityLinker):
    ...
    def candidate_decision_function(self, mention: MentionData, candidate_list: list[MentionData]) -> EntityData | None:
        ...
        logger.debug(f"Vector of entity {mention} is close to the vectors of {major_entity.entity_qid}")
        ...
        if similarity(mention.span, aliases) < self.cutoff_threshold:
            ...
            logger.debug(f"SUCC >> Entity recognized as one of {entity} aliases {aliases} ")
            return entity


    def link_entities(self, recognized_entities: list[MentionData], document: DocumentData) -> list[EntityData]:
        """
        TODO
        """
        ...
        for mention in recognized_entities:
            candidate_list = self.repository.get_similar_mentions(mention, max_mentions=self.k_neighbors)
